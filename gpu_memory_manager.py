...
class GPUMemoryManager:
    ...
    def report(self) -> MemoryReport:
        ...
        return MemoryReport(
            ...
            peak_allocated_gb=round(torch.cuda.max_memory_allocated(idx) / (1024**3), 4),
        )
    ...
    def safe_tensor(self, shape: tuple[int, ...], dtype: torch.dtype = torch.float32) -> torch.Tensor:
        ...
        if not self.preallocation_check(required_gb * 1.2):
            raise RuntimeError("OOM prevention triggered: requested tensor exceeds safe budget")
    ...
    def auto_tune_batch_size(self, trial_fn, low: int = 8, high: int = 4096, target_util: float = 0.88) -> int:
        ...
        while low <= high:
            ...
            try:
                ...
                if self.available():
                    idx = int(self.device.split(":")[-1])
                    util = torch.cuda.memory_reserved(idx) / torch.cuda.get_device_properties(idx).total_memory
                else:
                    util = 0.0
                if util <= target_util:
                    best = mid
    ...
    def suggest_gradient_accumulation(self, desired_effective_batch: int, max_batch: int) -> int:
        if max_batch <= 0:
            return 1
    ...
    def fragmentation_indicator(self) -> float:
        ...
        if rep.reserved_gb <= 0:
            return 0.0
        return round(max(0.0, (rep.reserved_gb - rep.allocated_gb) / rep.reserved_gb), 4)
